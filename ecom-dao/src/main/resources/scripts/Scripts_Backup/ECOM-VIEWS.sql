...
DATE(CREATED_DATE) AS CREATED_DATE, SUM(ITEM_SALE_PRICE) AS Revenue,SUM(ITEM_SALE_PRICE)+SUM(ITEM_TAX_AMOUNT) AS 'Revenue_tax_incl', SUM(ITEM_ORIGNAL_AMT) AS 'Cost_of_Goods', 
(SUM(ITEM_SALE_PRICE)-SUM(ITEM_ORIGNAL_AMT)) As 'Gross_Profit', 
...
CREATE VIEW Inventory_Report(ID,PRODUCT_NAME ,SKU ,SUPPLY_PRICE_EXCL_TAX,REORDER_POINT,REORDER_AMOUNT,CURRENT_INVENTORY,NET_PRICE,OUTLET_NAME , OUTLET_ASSOCICATION_ID, CREATED_DATE, 
...
SKU,SUPPLY_PRICE_EXCL_TAX,REORDER_POINT,REORDER_AMOUNT,CURRENT_INVENTORY
,(((SUPPLY_PRICE_EXCL_TAX*MARKUP_PRCT)/100)+SUPPLY_PRICE_EXCL_TAX) AS NET_PRICE,
(select OUTLET_NAME from  outlet WHERE OUTLET_ASSOCICATION_ID = OUTLET_ID) AS OUTLET_NAME ,
...
(select PRODUCT_NAME from ecom.product where PRODUCT_ASSOCIATION_ID=PRODUCT_ID) AS 'Product',
(select OUTLET_NAME from ecom.outlet where OUTLET_ASSOCICATION_ID=OUTLET_ID) AS 'Outlet',
(select CONTACT_NAME  from ecom.contact where CONTACT_TYPE LIKE '%CUSTOMER%' AND CONTACT_ID = (select CONTACT_ASSOCIATION_ID from ecom.invoice_main where INVOICE_MAIN_ASSOCICATION_ID=INVOICE_MAIN_ID)) AS 'Customer' ,
DATE(CREATED_DATE) AS CREATED_DATE, SUM(ITEM_SALE_PRICE) AS Revenue,SUM(ITEM_SALE_PRICE)+SUM(ITEM_TAX_AMOUNT) AS 'Revenue_tax_incl', SUM(ITEM_ORIGNAL_AMT) AS 'Cost_of_Goods', 
(SUM(ITEM_SALE_PRICE)-SUM(ITEM_ORIGNAL_AMT)) As 'Gross_Profit', 
(100-(SUM(ITEM_ORIGNAL_AMT)/SUM(ITEM_SALE_PRICE))*100) As Margin,
SUM(PRODUCT_QUANTITY) AS 'Items_Sold',
...
(select PRODUCT_NAME from ecom.product where PRODUCT_ASSOCIATION_ID=PRODUCT_ID) AS 'Product',
(select OUTLET_NAME from ecom.outlet where OUTLET_ASSOCICATION_ID=OUTLET_ID) AS 'Outlet',
(select CONTACT_GROUP_NAME from contact_group where CONTACT_GROUP_ID  = (select CONTACT_GROUP_ASSOCIATION_ID  from ecom.contact where CONTACT_TYPE LIKE '%CUSTOMER%' AND CONTACT_ID = (select CONTACT_ASSOCIATION_ID from ecom.invoice_main where INVOICE_MAIN_ASSOCICATION_ID=INVOICE_MAIN_ID))) AS 'Group_Name' ,
DATE(CREATED_DATE) AS CREATED_DATE, SUM(ITEM_SALE_PRICE) AS Revenue,SUM(ITEM_SALE_PRICE)+SUM(ITEM_TAX_AMOUNT) AS 'Revenue_tax_incl', SUM(ITEM_ORIGNAL_AMT) AS 'Cost_of_Goods', 
(SUM(ITEM_SALE_PRICE)-SUM(ITEM_ORIGNAL_AMT)) As 'Gross_Profit', 
(100-(SUM(ITEM_ORIGNAL_AMT)/SUM(ITEM_SALE_PRICE))*100) As Margin,
SUM(PRODUCT_QUANTITY) AS 'Items_Sold',
...
(select CONCAT(FIRST_NAME, ' ',LAST_NAME) from ecom.user where ecom.user.USER_ID=daily_register.UPDATED_BY) AS 'Close_By',
