...
/**
 * TagSalesReportController
 * @constructor
 */

var TagSalesReportController = ['$scope', '$http', '$window','$cookieStore','$rootScope','SessionService','$timeout','$interval','$route','$filter','TagSalesReportControllerPreLoad',function($scope, $http, $window,$cookieStore,$rootScope,SessionService,$timeout,$interval,$route,$filter,TagSalesReportControllerPreLoad) {
	...
	$scope.sesssionValidation = function(){
		if($rootScope.salesReportDateType == "weekly"){
			...
			$scope.data = TagSalesReportControllerPreLoad.loadControllerData();
			$scope.fetchData();
		}
	};
