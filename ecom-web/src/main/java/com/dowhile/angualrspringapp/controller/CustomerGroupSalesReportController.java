 ...
 */
@Controller
@RequestMapping("/customerGroupSalesReport")
public class CustomerGroupSalesReportController {
	...
	@RequestMapping(value = "/getCustomerGroupSalesReportByDateRange/{sessionId}/{startDate}/{endDate}/{reportType}/{reportDateType}/{outletName}/{isLiveData}", method = RequestMethod.POST)
	public @ResponseBody Response getCustomerGroupSalesReportByDateRange(@PathVariable("sessionId") String sessionId,
		...
		if(SessionValidator.isSessionValid(sessionId, request)){
			...
			try {
				...
				if(isLiveData!=null && !isLiveData.equalsIgnoreCase("") && isLiveData.equalsIgnoreCase("true")){
					tableName = new StringBuilder("Customer_Group_Sales_Report");
				}else{
					tableName = new StringBuilder("mv_Customer_Group_Sales_Report");
				}
				int outletId = 0;
				...
				if(reportType.equals("Revenue")){
					ReportParams reportParams = new ReportParams();
						...
						reportParams.setGroupBy("Group_Name");
						reportParams.setMainBaseColumn("Group_Name");
				...
				if (tableData != null) {
					...
					util.AuditTrail(request, currentUser, "CustomerGroupSalesReportController.getCustomerGroupSalesReportByDateRange", 
							"User "+ currentUser.getUserEmail()+" retrived brand sales report against date range successfully ",false);
					return new Response(reportControllerBean, StatusConstants.SUCCESS,
							LayOutPageConstants.STAY_ON_PAGE);
				} else {
					util.AuditTrail(request, currentUser, "CustomerGroupSalesReportController.getCustomerGroupSalesReportByDateRange", 
							" Customer group Sales report is not found requested by User "+currentUser.getUserEmail(),false);
		...
		if(SessionValidator.isSessionValid(sessionId, request)){
			...
			try {
				...
				if(outlets!=null){
					...
					util.AuditTrail(request, currentUser, "CustomerGroupSalesReportController.getOutlets", 
