			...
			try {
				...
				if(reportType.equals("Revenue")){
					ReportParams reportParams = new ReportParams();
						reportParams.setBaseColumn("sum(AMOUNT) as Amount");
				...
				if (tableData != null) {
					...
					util.AuditTrail(request, currentUser, "PaymentReportController.getPaymentReportByDateRange", 
							"User "+ currentUser.getUserEmail()+" retrived payment sales report against date range successfully ",false);
					return new Response(reportControllerBean, StatusConstants.SUCCESS,
							LayOutPageConstants.STAY_ON_PAGE);
				} else {
					util.AuditTrail(request, currentUser, "PaymentReportController.getPaymentReportByDateRange", 
							" Payment Sales report is not found requested by User "+currentUser.getUserEmail(),false);
