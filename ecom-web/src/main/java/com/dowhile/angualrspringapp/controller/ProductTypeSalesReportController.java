 ...
 */
@Controller
@RequestMapping("/productTypeSalesReport")
public class ProductTypeSalesReportController {

	private static Logger logger = Logger.getLogger(ProductTypeSalesReportController.class.getName());
	@Resource
	...
	@RequestMapping(value = "/geProductTypeSalesReportByDateRange/{sessionId}/{startDate}/{endDate}/{reportType}/{reportDateType}/{outletName}/{isLiveData}", method = RequestMethod.POST)
	public @ResponseBody Response geProductTypeSalesReportByDateRange(@PathVariable("sessionId") String sessionId,
		...
		if(SessionValidator.isSessionValid(sessionId, request)){
			...
			try {
				...
				if(isLiveData!=null && !isLiveData.equalsIgnoreCase("") && isLiveData.equalsIgnoreCase("true")||isLocalInstance){
					tableName = new StringBuilder("ProductType_Sales_Report");
				}else{
					tableName = new StringBuilder("mv_ProductType_Sales_Report");
				}
...
//				configuration = configurationMap.get("HIDE_ORIGNAL_PRICE_INFO_REPORTS");
				...
				if(reportType.equals("Revenue")){
					ReportParams reportParams = new ReportParams();
						...
						reportParams.setGroupBy("ProductType");
						reportParams.setMainBaseColumn("ProductType");
				...
				if (tableData != null) {
					...
					util.AuditTrail(request, currentUser, "ProductTypeSalesReportController.geProductTypeSalesReportByDateRange", 
							"User "+ currentUser.getUserEmail()+" retrived product type sales report against date range successfully ",false);
					return new Response(reportControllerBean, StatusConstants.SUCCESS,
							LayOutPageConstants.STAY_ON_PAGE);
				} else {
					util.AuditTrail(request, currentUser, "ProductTypeSalesReportController.geProductTypeSalesReportByDateRange", 
							"Product type Sales report is not found requested by User "+currentUser.getUserEmail(),false);
		...
		if(SessionValidator.isSessionValid(sessionId, request)){
			...
			try {
				...
				if(outlets!=null){
					...
					util.AuditTrail(request, currentUser, "ProductTypeSalesReportController.getOutlets", 
