...
var SalesReportController = ['$scope', '$http', '$window','$cookieStore','$rootScope','SessionService','$timeout','$interval','$route','$filter','SalesReportControllerPreLoad',function($scope, $http, $window,$cookieStore,$rootScope,SessionService,$timeout,$interval,$route,$filter,SalesReportControllerPreLoad) {
	...
	$scope.sesssionValidation = function(){
		if($rootScope.salesReportDateType == "weekly"){
			...
			$scope.data = SalesReportControllerPreLoad.loadControllerData();
			$scope.fetchData();
		}
	};
