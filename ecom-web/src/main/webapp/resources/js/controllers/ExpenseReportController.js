...
var ExpenseReportController = ['$scope', '$http', '$window','$cookieStore','$rootScope','SessionService','$timeout','$interval','$route','$filter','ExpenseReportControllerPreLoad',function($scope, $http, $window,$cookieStore,$rootScope,SessionService,$timeout,$interval,$route,$filter,ExpenseReportControllerPreLoad) {
	...
	$scope.reportTypeSelect = $rootScope.expenseReportType;
	...
	$scope.sesssionValidation = function(){
		if($rootScope.salesReportDateType == "weekly"){
			...
			$scope.data = ExpenseReportControllerPreLoad.loadControllerData();
			$scope.fetchData();
		}
	};
	...
	$scope.changeReportDataType = function() {
		$rootScope.expenseReportType = $scope.reportType;
		$route.reload();
	};
