...
/**
 * CashSalesReportController
 * @constructor
 */

var CashSalesReportController = ['$scope', '$http', '$window','$cookieStore','$rootScope','SessionService','$timeout','$interval','$route','$filter','CashSalesReportControllerPreLoad',function($scope, $http, $window,$cookieStore,$rootScope,SessionService,$timeout,$interval,$route,$filter,CashSalesReportControllerPreLoad) {
	...
	$scope.sesssionValidation = function(){
		if($rootScope.salesReportDateType == "weekly"){
			...
			$scope.data = CashSalesReportControllerPreLoad.loadControllerData();
			$scope.fetchData();
		}
	};
