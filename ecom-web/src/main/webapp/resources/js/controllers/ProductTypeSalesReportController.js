...
/**
 * ProductTypeSalesReportController
 * @constructor
 */

var ProductTypeSalesReportController =['$scope', '$http', '$window','$cookieStore','$rootScope','SessionService','$timeout','$interval','$route','$filter','ProductTypeSalesReportControllerPreLoad' ,function($scope, $http, $window,$cookieStore,$rootScope,SessionService,$timeout,$interval,$route,$filter,ProductTypeSalesReportControllerPreLoad) {
	...
	$scope.sesssionValidation = function(){
		if($rootScope.salesReportDateType == "weekly"){
			...
			$scope.data = ProductTypeSalesReportControllerPreLoad.loadControllerData();
			$scope.fetchData();
		}
	};
