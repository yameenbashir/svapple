 ...
 * @constructor
 */
var StockReturnController = ['$scope', '$http','$filter', '$window','$cookieStore','$rootScope','$timeout','SessionService','StockReturnControllerPreLoad',function($scope, $http,$filter, $window,$cookieStore,$rootScope,$timeout,SessionService,StockReturnControllerPreLoad) {
	...
	$scope.stockOrderBean.orderNo = "SR-"+String(parseInt(new Date().getMonth())+1)+"/"+new Date().getDate()+"/"+new Date().getFullYear()+ " " + $scope.HHMM;
	$scope.stockOrderBean.stockRefNo = "SR-"+String(parseInt(new Date().getMonth())+1)+"/"+new Date().getDate()+"/"+new Date().getFullYear()+ " " + $scope.HHMM;
	$scope.sessionValidation = function(){
		if(SessionService.validate()){
			...
			$scope.data = StockReturnControllerPreLoad.loadControllerData();
			$scope.fetchData();
			$scope.sReturn = '1';
			$scope.isAdmin();
			if($scope.sReturn != '1'){
	...
	$scope.addStockOrder = function() {		
		...
		$scope.stockOrderBean.stockOrderTypeId = "2"; // Supplier Return
