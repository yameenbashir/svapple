...
/**
 * CustomerGroupSalesReportController
 * @constructor
 */

var CustomerGroupSalesReportController = ['$scope', '$http', '$window','$cookieStore','$rootScope','SessionService','$timeout','$interval','$route','$filter','CustomerGroupSalesReportControllerPreLoad',function($scope, $http, $window,$cookieStore,$rootScope,SessionService,$timeout,$interval,$route,$filter,CustomerGroupSalesReportControllerPreLoad) {
	...
	$scope.sesssionValidation = function(){
		if($rootScope.salesReportDateType == "weekly"){
			...
			$scope.data = CustomerGroupSalesReportControllerPreLoad.loadControllerData();
			$scope.fetchData();
		}
	};
